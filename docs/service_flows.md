...
5. **ETL Service** (Data Engineering) mendengarkan event ini:
   ...
   - Mengisi dimensi `dim_order`, `dim_service`, dan `dim_date` di Data Warehouse.
   - Membuat baris baru di tabel fakta `fact_shipment` (dengan status kurir `-1` / Belum Ditugaskan).
...
4. **ETL Service** mendengarkan event ini:
   ...
   - Memperbarui tabel dimensi `dim_courier` di DWH.
   - Memperbarui `courier_key` di tabel fakta `fact_shipment` untuk menghubungkan pesanan tersebut dengan kurir bersangkutan.
...
| :--- | :--- | :--- | :--- | :--- |
| `papiton.events.order` | `order.created` | Order & Tariff Service | Notification Service | Membuat baris di `dim_order`, `dim_service`, dan `fact_shipment` (initial record). |
| `papiton.events.shipping` | `package.picked_up` / `dispatch.assigned` | Shipping & Dispatch Service | Notification Service | Mengisi profil kurir di `dim_courier` dan memperbarui `courier_key` di `fact_shipment`. |
